...
// only maps str->T, doesn't allow for deleting keys, cause thats for losers
...
public:
    ...
    Hashmap(std::initializer_list<std::pair<const char *, T>> list)
        : Hashmap(0.7, list.size() + 1) {
    ...
    Hashmap(const Hashmap<T> &other) {
        ...
        first_entry = nullptr;
        last_entry = nullptr;
        _map_key = (KeyEntry *)calloc(sizeof(KeyEntry), size + 1);
        _map_value.resize(size + 1);
        for (std::pair<char *, T> item : other.items()) {
            set(std::string(item.first), item.second);
        }
    }
